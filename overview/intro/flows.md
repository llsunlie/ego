...
│    │     → 后端保存 Moment                                │
│    │     → 后端语义匹配历史 → 返回 Echo (含 candidates)     │
│    │     → 前端渲染 EchoCard                             │
│    ├─ [API] Ego.GenerateInsight(content, echo_moment_id) │
│    │     → 后端 AI 生成第二人称观察                        │
│    │     → 前端渲染 InsightCard                          │
│    └─ 两个接口并行调用，EchoCard 先出，InsightCard 紧随    │
│                                                         │
│ 4. 用户看到三出口按钮                                     │
│    ├─ "顺着再想想"（金色主按钮）→ F2                       │
│    ├─ "✦ 收进星图" → F3                                  │
...
**接口调用序列：**
```
CreateMoment ──┬── 成功 → 渲染 EchoCard
               │
GenerateInsight ── 成功 → 渲染 InsightCard
               │
...
**接口调用序列：**
```
第 1 轮: CreateMoment("今天和同事...") + GenerateInsight
第 2 轮: CreateMoment("其实是我害怕...", trace_id=T1) + GenerateInsight
第 3 轮: CreateMoment("小时候...", trace_id=T1) + GenerateInsight
...
...
│    └─ 坐标归一化为 0-1                                   │
│                                                         │
│ 3. [API] Ego.StashTrace(trace_id, x, y)                 │
│    ├─ 后端：                                              │
│    │   ├─ 标记该 Trace 已寄存（创建 Star 记录）             │
│    │   ├─ 创建 Star（位置 x,y，初始视觉状态）              │
│    │   ├─ 重新评估相关 Constellation 状态                 │
│    │   │   ├─ 新星可能与已有星座聚类 → formed/forming 变更 │
│    │   │   ├─ 新星可能独立 → lone                         │
│    │   │   └─ 两颗孤星可能凑成 forming                     │
│    │   └─ 返回 Star                                     │
│    │                                                     │
...
┌─────────────────────────────────────────────────────────┐
│ 1. 切换到"过往"Tab                                        │
│    ├─ [API] Ego.ListMoments(cursor="", page_size=20)     │
│    │     → 后端返回扁平列表，前端按 created_at 自行按月分组 │
│    │     → 每条 Moment 含 connected 字段（是否已联结）      │
│    └─ 前端渲染时间线                                      │
│                                                         │
│ 2. 滚动到底部（加载更多）                                  │
│    ├─ [API] Ego.ListMoments(cursor=last_id, page_size=20)│
│    └─ 追加到列表尾部                                      │
│                                                         │
│ 3. 点击某条 Moment                                       │
│    └─ AnimatedSize 展开 → 显示完整内容                     │
│       （纯前端，数据已在列表中）                            │
│                                                         │
│ 4. ✦ 已联结 标记                                         │
│    └─ Moment.connected == true 时显示                     │
│       此字段由 StashTrace 后后端更新                       │
└─────────────────────────────────────────────────────────┘
```

**覆盖状态：** `ListMoments` ✓（游标分页，前端按月分组）
...
│    │     → 返回所有 Constellation（含 Star 坐标/视觉状态）  │
...
```
┌─────────────────────────────────────────────────────────┐
│ 1. 用户在详情页看到 PastSelfCard × 3                      │
│    ├─ 每张卡片展示时段范围（如"2月-3月"）+ 开场白摘录       │
│    └─ 数据来自 Ego.GetConstellation 返回的 past_self_cards │
│                                                         │
│ 2. 点击一张卡片 → 拉起对话浮层                              │
│    ├─ [API] Ego.StartChat(past_self_card_id,             │
│    │                       context_moment_ids)            │
│    │     → 后端构建 past-self 上下文                      │
│    │        ├─ 原话（思想灵魂）— 不可伪造                   │
│    │        ├─ 延伸（语气/风格）— 灵动回应当下               │
...
```
┌─────────────────────────────────────────────────────────┐
│ 1. 用户在详情页看到 TopicPromptCard × 3                   │
│    ├─ 每个引子："顺着 X月X日：'你说过...' — 那'...'对你   │
│    │             到底意味着什么？"                         │
│    └─ 数据来自 Ego.GetConstellation 返回的 topic_prompts  │
│                                                         │
│ 2. 点击话题引子卡片                                       │
│    ├─ Navigator.pop() 回到星图 Tab                        │
│    ├─ 切换到 NowPage Tab                                 │
│    └─ 携带 topic: { anchor_moment, question } 进入写字区   │
│       （纯前端路由传参，不调接口）                          │
│                                                         │
│ 3. 用户看到输入区，placeholder 携带话题上下文               │
│    └─ 用户写完后走完整 F1 主流程                          │
│                                                         │
│ CreateMoment 时带上 topic 字段                            │
│ └─ 后端可将话题内容与回复关联，用于后续洞察                 │
└─────────────────────────────────────────────────────────┘
```

**覆盖状态：** `GetConstellation`（已含 topic_prompts）✓

> `CreateMomentReq.topic` 字段已在 proto 中定义，后端接收后当前不持久化，预留用于未来洞察关联。
...
│    │                                                     │
│    ├─ [API] Ego.GenerateInsight(content)                 │
│    │     → 无回声可参照 → insight: null 或通用安抚文本      │
│    │     → 不显示 InsightCard                             │
│    └─ 只显示三个出口按钮                                   │
│                                                         │
│ 4. "收进星图" → 第一颗星                                   │
│    ├─ [API] Ego.StashTrace(trace_id, x, y)               │
│    │     → 创建第一颗 Star                                │
│    │     → status: lone（孤星，等待同伴）                   │
│    └─ 星图 Tab → 已有 1 颗星                              │
│                                                         │
