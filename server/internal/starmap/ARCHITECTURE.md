...
| Layer | Role |
| --- | --- |
| `domain/` | Domain types (Star, Constellation, TraceProfile, ConstellationProfile) + ports (repositories, TraceStasher, TraceProfileGenerator, ConstellationAssetGenerator) |
| `app/` | Use cases: StashTrace, ListConstellations, GetConstellation + profile-based clustering and asset policy orchestration |
...
bootstrap/starmap.go → starmap.Deps{DB} → starmap/module.go
  ...
  → starmap app policies (TraceProfileGenerator, ConstellationProfile matching, ConstellationAssetGenerator)
