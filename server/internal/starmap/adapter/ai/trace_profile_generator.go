...
func (g *TraceProfileGenerator) Generate(ctx context.Context, trace writingdomain.Trace, moments []writingdomain.Moment) (*domain.TraceProfile, *domain.TraceProfileVector, error) {
	...
	embedding, err := g.createEmbeddingWithRetry(ctx, profile.ProfileText)
	if err != nil {
...
func (g *TraceProfileGenerator) createEmbeddingWithRetry(ctx context.Context, input string) (*platformai.EmbeddingResult, error) {
	return g.client.CreateEmbeddingWithRetry(ctx, input, platformai.RetryOptions{
		MaxAttempts: traceProfileEmbeddingMaxAttempts,
		Backoff:     traceProfileEmbeddingRetryBackoff,
