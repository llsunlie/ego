...
func TestParseTraceProfileJSON_StripsMarkdownFence(t *testing.T) {
	resp, err := parseTraceProfileJSON("```json\n{\"topic\":\"关系等待\",\"summary\":\"用户记录了不想总是先开口但仍在等待对方发现。\",\"keywords\":[\"先开口\",\"等待\"],\"emotions\":[\"疲惫\"],\"scenes\":[\"亲密关系\"],\"central_pattern\":\"在关系里等待对方主动靠近。\",\"representative_moment_id\":\"m1\"}\n```")
	if err != nil {
...
func TestNormalizeRepresentativeMomentID_FallsBackToFirstMoment(t *testing.T) {
	...
	got := normalizeRepresentativeMomentID("not-found", moments)
	if got != "m1" {
...
func TestBuildTraceProfileUserPrompt_OmitsEmptyMotivationAndKeepsOrder(t *testing.T) {
	...
	if !strings.Contains(prompt, "1. id=m1\ncontent:\n我不想每次都是我先开口。") {
		t.Fatalf("prompt missing first moment content: %s", prompt)
	}
	if !strings.Contains(prompt, "2. id=m2\ncontent:\n但我又一直在等他发现。") {
		t.Fatalf("prompt missing second moment content: %s", prompt)
	}
	if strings.Index(prompt, "id=m1") > strings.Index(prompt, "id=m2") {
