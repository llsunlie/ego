...
| 文件 | 说明 |
|------|------|
| `client/lib/features/now/widgets/starry_background.dart` | 星空背景粒子（共享组件，也被 login、onboarding、setting 模块使用） |
...
| `client/lib/features/now/widgets/guide_section.dart` | idle 状态引导文字 (GuideText: 7 条文案打字机轮播) + 底部 WriteButton |
...
> **StarryBackground 跨模块共享**：该组件最初属于 now 模块，现已被以下页面复用：
> - `client/lib/features/login/login_page.dart`
> - `client/lib/features/onboarding/onboarding_page.dart`
> - `client/lib/features/setting/setting_page.dart`
